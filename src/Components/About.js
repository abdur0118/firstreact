...
export default function About() {
    ...
    let toggleStyle = ()=>{
        ...
        if(myStyle.backgroundColor === 'white'){
            myStylet.backgroundColor = 'black'
            myStylet.color = 'white'
            setMyStyle(myStylet)
            setBtnTxt('Light')
        }
        else{
            myStylet.backgroundColor = 'white'
            myStylet.color = 'black'
            setMyStyle(myStylet)
            setBtnTxt('Dark')
        }
    }
  return (
      <div className="container" style={myStyle}>
        ...
        <button type='button' className={`btn btn-${btnTxt.toLowerCase()} my-3`} onClick={toggleStyle}>Enable {btnTxt} mode</button>
      </div>
  );
}
